...
// Board is detected from the framework's own auto-defined macros (TEENSYDUINO / ARDUINO_ARCH_SAMD),
// so no custom -D build flag is required in platformio.ini -- just pick the matching env.
#if defined(TEENSYDUINO)
#define PLATFORM_TEENSY 1
#elif defined(ARDUINO_ARCH_SAMD)
#define PLATFORM_SAMD51 1
#else
#error "Unsupported platform: this file targets Teensy 3.x (TeensyDuino) or the Adafruit Feather M4 Express (SAMD51) only."
#endif
...
#if defined(PLATFORM_SAMD51)
DeadbandMinMax deadbandMinMax4096 = {600, 3400}; // default; may be overwritten by the saved persisted values
#elif defined(PLATFORM_TEENSY)
DeadbandMinMax deadbandMinMax4096 = {504, 3760}; // recalced fromdeadbandMinMax1024 >> should/could be suitable values for eel #1
#endif

// Persists deadbandMinMax4096 across power cycles: FlashStorage_SAMD (flash-emulated) on the SAMD51,
// EEPROM (native, non-emulated) on Teensy. The signature marks whether storage actually holds a
// previously-saved value, since erased/unwritten storage won't reliably read back as this struct's zero value.
struct DeadbandMinMax4096Storage
{
    uint16_t min;
    uint16_t max;
    uint32_t signature;
};
#define DEADBAND_MIN_MAX_4096_STORAGE_SIGNATURE 0x5A5AA5A5

#if defined(PLATFORM_SAMD51)
FlashStorage(deadbandMinMax4096FlashStore, DeadbandMinMax4096Storage);
#elif defined(PLATFORM_TEENSY)
#define EEPROM_DEADBAND_MIN_MAX_4096_ADDR 0
#endif

bool deadbandMinMax4096Dirty = false;
unsigned long deadbandMinMax4096LastChangeMillis = 0;
const unsigned long deadbandMinMax4096SaveDelay = 5000; // wait for this many quiet ms before committing to storage, to avoid wearing it out
...
#if defined(PLATFORM_SAMD51)
uint8_t dac1_pin = A1; // n00d output -- true 12-bit DAC
uint8_t dac2_pin = A0; // audio output -- true 12-bit DAC
uint8_t pwm1_pin = 4;
uint8_t pwm2_pin = 0;
uint8_t pwm3_pin = 1;
#elif defined(PLATFORM_TEENSY)
uint8_t dac1_pin = A14; // n00d output -- Teensy 3.x's only true 12-bit DAC
uint8_t dac2_pin = 20;  // audio output -- Teensy has no second DAC, so this is PWM instead
uint8_t pwm1_pin = 22;
uint8_t pwm2_pin = 23;
uint8_t pwm3_pin = 6;
#endif
...
#endif
// On Teensy, USB-MIDI is provided by the core itself as the global `usbMIDI` object
// (enabled via the -D USB_MIDI_SERIAL build flag), no library/instance needed.
...
void loadDeadbandMinMax4096FromStorage();
void markDeadbandMinMax4096Changed();
void updateDeadbandMinMax4096StorageSave();
...
void setup()
{
    ...
    loadDeadbandMinMax4096FromStorage();
...
void loop()
{
    ...
    MIDI.read();
#elif defined(PLATFORM_TEENSY)
    // usbMIDI.read() needs to be called rapidly from loop(). It also dispatches
    // to the handlers registered in setup() (handleControlChange / handleNoteOn).
    usbMIDI.read();
#endif
    ...
    // the built-in LED lights up while calibration values are being sent
    ...
    updateDeadbandMinMax4096StorageSave();
}
...
void loadDeadbandMinMax4096FromStorage()
{
    DeadbandMinMax4096Storage stored;
#if defined(PLATFORM_SAMD51)
    deadbandMinMax4096FlashStore.read(stored);
#elif defined(PLATFORM_TEENSY)
    EEPROM.get(EEPROM_DEADBAND_MIN_MAX_4096_ADDR, stored);
#endif

    if (stored.signature == DEADBAND_MIN_MAX_4096_STORAGE_SIGNATURE)
    {
        deadbandMinMax4096.min = stored.min;
        deadbandMinMax4096.max = stored.max;
        Serial.println("Loaded deadbandMinMax4096 from storage -> min: " + String(deadbandMinMax4096.min) + ", max: " + String(deadbandMinMax4096.max));
    }
    else
    {
        Serial.println("No saved deadbandMinMax4096 found in storage, using defaults -> min: " + String(deadbandMinMax4096.min) + ", max: " + String(deadbandMinMax4096.max));
    }
}
...
void updateDeadbandMinMax4096StorageSave()
{
    ...
    {
        DeadbandMinMax4096Storage toStore;
        toStore.min = deadbandMinMax4096.min;
        toStore.max = deadbandMinMax4096.max;
        toStore.signature = DEADBAND_MIN_MAX_4096_STORAGE_SIGNATURE;

#if defined(PLATFORM_SAMD51)
        deadbandMinMax4096FlashStore.write(toStore);
#elif defined(PLATFORM_TEENSY)
        EEPROM.put(EEPROM_DEADBAND_MIN_MAX_4096_ADDR, toStore);
#endif

        deadbandMinMax4096Dirty = false;
        Serial.println("Saved deadbandMinMax4096 to storage -> min: " + String(deadbandMinMax4096.min) + ", max: " + String(deadbandMinMax4096.max));
    }
}
