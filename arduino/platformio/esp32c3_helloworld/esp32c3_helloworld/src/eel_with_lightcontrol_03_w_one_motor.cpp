...
void loop()
{
  ...
  EVERY_N_MILLIS(200)
  {
    ...
    {
      // Serial.println("motor 1 go forward");
      ledcDetachPin(motor1b_pin);
      digitalWrite(motor1b_pin, 0);
      ledcDetachPin(motor2b_pin);
      digitalWrite(motor2b_pin, 0);
      ...
      ledcWrite(motor1_chan, map(data.ch[TX_PITCH], SBUS_VAL_CENTER, SBUS_VAL_MAX, 0, 255));
      ledcAttachPin(motor2a_pin, motor2_chan);
      ledcWrite(motor2_chan, map(data.ch[TX_PITCH], SBUS_VAL_CENTER, SBUS_VAL_MAX, 0, 255));
    }
    else if (data.ch[TX_PITCH] < (SBUS_VAL_CENTER - SBUS_VAL_DEADBAND))
    {
      // Serial.println("motor 1 go reverse");
      ledcDetachPin(motor1a_pin);
      digitalWrite(motor1a_pin, 0);
      ledcDetachPin(motor2a_pin);
      digitalWrite(motor2a_pin, 0);
      ...
      ledcWrite(motor1_chan, map(data.ch[TX_PITCH], SBUS_VAL_CENTER, SBUS_VAL_MIN, 0, 255));
      ledcAttachPin(motor2b_pin, motor2_chan);
      ledcWrite(motor2_chan, map(data.ch[TX_PITCH], SBUS_VAL_CENTER, SBUS_VAL_MIN, 0, 255));
    }
    else
    {
  ...
  if (data.ch[TX_PITCH] > (SBUS_VAL_CENTER + SBUS_VAL_DEADBAND))
  {
    ...
    motorA.motorGo(map(data.ch[TX_PITCH], SBUS_VAL_CENTER, SBUS_VAL_MAX, 0, 255));
  }
  else if (data.ch[TX_PITCH] < (SBUS_VAL_CENTER - SBUS_VAL_DEADBAND))
  {
    motorA.motorRev(map(data.ch[TX_PITCH], SBUS_VAL_CENTER, SBUS_VAL_MIN, 0, -255));
  }
  else
  {
    motorA.motorStop();
